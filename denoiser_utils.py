...
def denoise(config_path):

    seed_everything(0)

    BASE_PATH = 'configs/test/denoise.yaml'
    SKIP_LARGE = True
    CUT_LARGE = True
    DEVICE = 'cpu'
    OVERWRITE = False
    ...
    write_path = os.path.join(config.write_path, '')

    pred_paths, cond_paths = sorted(glob.glob(config.pred_path)), sorted(glob.glob(config.cond_path))
    print("Number of images in folders: ", len(pred_paths), len(cond_paths), config.pred_path, config.cond_path)
